...
```{r plot-to-replicate, echo=FALSE, fig.show="asis", fig.width=6.5, fig.height=3.8}
dl_aa_ua <- nycflights %>%
...
nycflights<- nycflights %>%
  mutate(avg_speed = (nycflights$distance*60/nycflights$air_time))

nyc_avg_speed <- nycflights%>% select(distance,air_time,avg_speed)
nyc_avg_speed
...
```{r echo=FALSE, fig.show="asis", fig.width=7, fig.height=4}
nycflights %>%
  ggplot(aes(distance,avg_speed,color=carrier)) + geom_point()+
  labs(x="Distance",y="Average Speed (mph)",title="Scatterplot of Average Speed vs Distance")

```

The relationship between distance average speed and distance is positive. avg_speed increase as distance increases.

### Solution 9

##  Estimating the cutoff point for departure delay where you can still expect to get to destination on time

```{r}
nycflights %>% filter(carrier == "AA" | carrier == "DL" | carrier == "UA")%>%
  ggplot(aes(x = dep_delay, y = arr_delay, color = carrier)) +  geom_hline(yintercept = 0)+ 
  geom_vline(xintercept = 0)+
  geom_point()+coord_cartesian(xlim = c(-10, 100))
```

A flight can have a departure delay of anytime greater than zero and less than approximately 62 minutes without having an arrival delay.
